...
\lstinputlisting[firstline=86,lastline=103,caption=Summing salaries in EMGM,label=src:emgm-company:sumSalary]{scala/src/emgm-company.scala}
\end{example}
...
We consider a basic transformation as a function of type \cd{A => A}. In EMGM,
we can encode this pattern as:
\lstinputlisting[firstline=274,lastline=290,caption=Basic transform pattern,label=src:emgm:GTransform]{scala/src/emgm.scala}

We can now simply extend that trait, as listing~\ref{src:emgm-company:incSalary} shows.
\lstinputlisting[firstline=78,lastline=83,caption=Using the transform pattern: Increase salary,label=src:emgm-company:incSalary]{scala/src/emgm-company.scala}

This breaks though if you want to have any other transformation than the Salary
one, because Scala would not be able to infer the correct \cd{GRep}.
\end{example}
