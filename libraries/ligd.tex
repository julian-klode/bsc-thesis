...
\lstinputlisting[firstline=52,lastline=67,float=ht,caption=LIGD Basics,label=ligd-base]{scala/src/ligd.scala}
...
implemented.
\lstinputlisting[firstline=75,lastline=89,float=ht,caption=Implementation of RType,label=ligd-rtype]{scala/src/ligd.scala}
...
%\lstinputlisting[firstline=103,lastline=107]{scala/src/ligd.scala}

\lstinputlisting[firstline=95,lastline=116,float=ht,caption=Implementation of rList,label=ligd-rlist]{scala/src/ligd.scala}
...
\begin{enumerate}
  \item Convert \cd{geq :: Rep[a] -> a -> a} to
        \cd{geq\_ :: Rep[a] -> a -> a -> (forall a. Rep[a] -> a -> a)}
        and change self-recursive calls to call the new argument
  \item Add a  \cd{geq rep a b = geq\_ rep a b geq}
\end{enumerate}
