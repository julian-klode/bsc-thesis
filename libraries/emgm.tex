...
\lstinputlisting[firstline=172,lastline=192,label=src:emgm:GEq,float=ht,caption=A generic equality function in GM]{scala/src/emgm.scala}
...
\lstinputlisting[firstline=58,lastline=70,label=src:emgm:Rep,caption=The \cd{Rep} trait in GM and some instances]{scala/src/emgm.scala}
...
\lstinputlisting[firstline=238,lastline=240,label=src:emgm:GenericList,caption=Generic with ad-hoc list case]{scala/src/emgm.scala}
...
\lstinputlisting[firstline=251,lastline=268,float=ht,label=src:emgm:GEqList,caption=Generic equality specialised to lists]{scala/src/emgm.scala}
...
generic functions.
\lstinputlisting[firstline=200,lastline=202,label=src:emgm:GRep,caption=The \cd{GRep} dispatcher]{scala/src/emgm.scala}

The instances of \cd{GRep} are basically the same as those of \cd{Rep}, except
that they capture the \cd{Generic[G]} in the constructor, rather than the
method.

\lstinputlisting[firstline=232,lastline=235,label=src:emgm:GRProd,caption=Example of a \cd{GRep} instance: \cd{GRProd}]{scala/src/emgm.scala}

We can now add a representation for our special case of lists:

\lstinputlisting[firstline=243,lastline=246,label=src:emgm:GRList,caption=Example of a \cd{GRep} instance: \cd{GRList}]{scala/src/emgm.scala}
